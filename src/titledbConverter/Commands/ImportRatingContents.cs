...
public class ImportRatingContents : AsyncCommand<ImportCategories.Settings>
{
    private readonly IImportTitleService _importTitleService;
    private readonly IOptions<AppSettings> _configuration;


    public ImportRatingContents(IImportTitleService importTitleService, IOptions<AppSettings> configuration)
    {
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, ImportCategories.Settings settings)
    {
        ...
        {
            await _importTitleService.ImportRatingContents(settings.ImportFile);
        }
